...
void matrixQR(float* A, int rows, int cols, float* Q, float* R){
    float* v = new float[rows]; // temp vector for calcs
    float* u = new float[rows]; // 2nd temp vector
    float* q = new float[rows]; // stores Q columns

    // Initializing Q and R to zero
    ...
    for(int j = 0; j < cols; j++){
        // Extract column J from A into v
        matrixExtractCol(A, rows, cols, j, v);
        // Then copy v into u for modifying
        for(int i = 0; i < rows; i++){
            u[i] = v[i];
        }
        // Subtract projections of our previous vectors
        for(int i = 0; i < j; i++){
            matrixExtractCol(Q, rows, cols, i, q);
            R[i * cols + j] = vectorDotProduct(q, v, rows);
            float* temp = new float[rows];
            vectorScale(q, rows, R[i * cols + j], temp);
            vectorSubtract(u, temp, rows, u);
            delete[] temp;
        }
        // Calculate R[j][j]
        float norm = vectorNorm(u, rows);
        R[j * cols + j] = norm;
        // Calculate Q's column j
        if(norm > 1e-10){ // Cchecking for non-zero norm, also using 0.0000000001 instead of 0 bc of floating point precision
            vectorScale(u, rows, 1.0/norm, q);
            for(int i = 0; i < rows; i++){
                Q[i * cols + j] = q[i];
            }
        }
